...
class ResumeScreener:
    ...
    def __init__(
        ...
        """
        load_dotenv()
        api_key_value = api_key or os.getenv("GROQ_API_KEY")
        if not api_key_value:
            raise RuntimeError(
                "Groq API key not provided. Set GROQ_API_KEY environment variable or pass api_key to ResumeScreener()."
            )
        groq_client = Groq(api_key=api_key_value)
    ...
    def screen_resume(
        ...
        Be flexible with synonyms: treat equivalent skills (e.g. SQL/MySQL, NoSQL/MongoDB) as matches.
        Use reasoning to identify conceptual overlaps even if names differ.
        ...
        10. Consider synonyms and conceptual overlaps (e.g., SQL ↔ MySQL, NoSQL ↔ MongoDB, ML ↔ Machine Learning). Reward conceptual matches
        11. Infer missing but implied skills (e.g., Data Pipelines → Airflow, ETL → SQL).
Reward resumes demonstrating transferable skills even if not explicitly listed.
        12. Identify which categories (skills, experience, education, etc.) are most emphasized in this JD and return weight suggestions.
        ...
        Identify which categories (skills, experience, education, etc.) are most emphasized in this JD and return weight suggestions
        """
