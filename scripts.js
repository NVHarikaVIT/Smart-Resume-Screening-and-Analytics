...
async function handleOptimization() {
  const optimizeBtn = document.getElementById("optimize-btn");
  const container = document.getElementById("resume-optimisation");
  ...
  try {
    const response = await fetch("http://localhost:5000/api/optimize", {
      ...
      body: JSON.stringify({
        parsed_resume: currentResults.parsed,
    ...
    if (result.success) {
      ...
      container.innerHTML = `
        <div class="fade-in" style="line-height:1.6;">
          ${formatted}
        </div>`;
    } else {
      container.innerHTML = `<p style="color:var(--color-error);">${result.error}</p>`;
    }
  } catch (error) {
    container.innerHTML = `<p style="color:var(--color-error);">Error generating suggestions: ${error.message}</p>`;
  } finally {
    optimizeBtn.disabled = false;
    optimizeBtn.textContent = "Generate Optimization Suggestions";
  }
}
...
function renderResults() {
  ...
  // Render resume optimisation suggestions
  renderOptimizationSuggestions(currentResults.optimisation);
}
...
function renderOptimizationSuggestions(optimisationText) {
  const container = document.getElementById("resume-optimisation");
  ...
  // Convert LLM bullet points to HTML
  const formatted = optimisationText
    .replace(/\n/g, "<br>")
    .replace(/- /g, "• ");
